  ...
  getDefaultData() {
    return {
      ...
      members: [{
        ...
        // passwordHash will be computed on first login
      }, {
        studentId: 'officer1',
        name: 'Maria Santos',
        year: '3',
...
const Auth = {
  ...
  async login(studentId, password, role) {
    ...
    if (member) {
      // If plain text password exists (legacy), convert to hash
...
// ================================================================
// MODULE: Gamification (EXP, Levels, Badges)
// ================================================================
const Gamification = {
  LEVELS: [
...
// ================================================================
// VIEW: Events (with event delegation)
// ================================================================
const Events = {
  render() {
    const data = Storage.getAppData();
    ...
    // Event delegation for check-in buttons (attached once in App.init)
  },
...
// ================================================================
// VIEW: Officer (dashboard, members, events management, etc.)
// ================================================================
const Officer = {
  renderDashboard() { /* ... same as before, with sanitizeHTML */ },
  renderMembers() { /* ... same */ },
  renderManageEvents() { /* ... same */ },
  renderAnnouncements() { /* ... same */ },
  renderBadges() { /* ... same */ },
  renderGradeRequests() { /* ... same */ },
  renderGrievances() { Grievance.render(); },
  renderLeaderboard() { Leaderboard.render(); },
  renderAnalytics() { /* ... same */ },
...
// (All Officer render methods are omitted for brevity – they are the same as in the original, just with sanitizeHTML applied to dynamic text.)
...
const App = {
  init() {
    ...
    // Set up event delegation for check-in buttons
    ...
    // Event delegation for search and filter (already in DOM)
  ...
  handleQRCheckin() {
    ...
    reader.start(
      ...
      (decodedText) => {
        // decodedText should be the event ID
        reader.stop();
        UI.closeModal();
