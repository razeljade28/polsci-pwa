  ...
  getAppData() {
    ...
    if (!data) {
      ...
      data.members.forEach((member) => {
        if (!member.role) {
          member.role = ["President", "Secretary"].includes(member.position)
            ? "officer"
            : "member";
        }
      });
      ...
      if (!data.learningResources?.sepolscisConstitution) {
        data.learningResources = {
          ...data.learningResources,
          sepolscisConstitution: learningResources.sepolscisConstitution,
        };
      }
      if (!data.opportunities)
        data.opportunities = this.getDefaultData().opportunities;
      ...
      if (
        data.learningResources &&
        data.learningResources.constitution ===
          "The Political Science Student Organization Constitution..."
      ) {
  ...
  getDefaultData() {
    return {
      events: [
        ...
        {
          id: 3,
          title: "Basketball Intrams",
          date: "2026-08-01",
          type: "sports",
          exp: 75,
          location: "Gymnasium",
          description: "Intramural basketball tournament.",
        },
        {
          id: 4,
          title: "Quiz Bee",
      ...
      opportunities: [
        {
          id: "o1",
          type: "Scholarship",
          title: "Future Public Leaders Scholarship",
          organization: "Civic Futures Foundation",
          deadline: "2026-07-30",
          location: "Online application",
          description:
            "Tuition support for students with a record of community leadership.",
          savedBy: [],
        },
        {
          id: "o2",
          type: "Internship",
          title: "Local Government Policy Internship",
          organization: "City Legislative Office",
          deadline: "2026-08-05",
          location: "City Hall",
          description:
            "A four-week placement supporting policy research and public consultations.",
          savedBy: [],
        },
        {
          id: "o3",
          type: "Debate",
          title: "Inter-University Debate Invitational",
          organization: "Philippine Debate Union",
          deadline: "2026-08-12",
          location: "University Auditorium",
          description:
            "Open call for student debate teams and adjudicator volunteers.",
          savedBy: [],
        },
        {
          id: "o4",
          type: "Volunteer",
          title: "Voter Education Weekend",
          organization: "Youth Civic Network",
          deadline: "2026-08-18",
          location: "Barangay Learning Hub",
          description: "Help facilitate non-partisan voter education sessions.",
          savedBy: [],
        },
      ],
      members: [
      ...
      grievances: [
        ...
        },
        {
          ...
          description:
            "The debate venue is too small for the expected participants.",
          anonymous: true,
          status: "in-progress",
      ...
      quests: [
        ...
        {
          id: "q2",
          title: "Read a learning resource",
          requirement: "read",
          progress: 0,
          target: 1,
          expReward: 15,
      ...
      if (q.requirement === requirement && !q.completed) {
        ...
        if (newProgress >= q.target) {
          ...
          const member = data.members.find(
            (m) => m.studentId === this.getCurrentUser()?.studentId
          );
          if (member) {
            member.exp += q.expReward;
            NotifCenter.add(
              "Quest Complete!",
              `You earned ${q.expReward} EXP for "${q.title}"`,
              "success",
              "check_circle"
            );
...
const Auth = {
  ...
  async login(studentId, password) {
    ...
    if (member) {
      ...
      return {
        success: false,
        message: "Account not found. Please create a student account first.",
      };
    }
  ...
  isOfficer() {
    return (
      this.currentUser &&
      ["officer", "adviser", "grievance"].includes(this.currentUser.role)
    );
  },
...
// UI helpers are loaded from ./ui.full.js
...
const NotifCenter = {
  render() {
    ...
    html += `</div>`;
    html += `<button class="btn-primary" style="margin-top:16px;width:auto;padding:12px 28px;display:inline-flex;" data-close-modal>Close</button>`;
  ...
  getBadgeDefinitions() {
    return [
      ...
      },
      {
        ...
        name: "Best Debater",
        icon: "🎤",
        condition: 'achievements.includes("Debate Winner")',
      ...
      },
      {
        ...
        name: "MVP",
        icon: "🏀",
        condition: 'achievements.includes("Sports MVP")',
        desc: "MVP in sports",
      },
      {
        id: "b6",
        name: "Quiz Bee Champ",
        icon: "🏆",
        condition: 'achievements.includes("Quiz Bee Champion")',
      ...
      },
      {
        ...
        icon: "🤝",
        condition: "exp >= 200",
        desc: "Reached 200 EXP",
      },
      {
        id: "b8",
        name: "President",
        icon: "👑",
        condition: 'position === "President"',
  ...
  checkBadges(studentId) {
    ...
    definitions.forEach((def) => {
      ...
      try {
        ...
        )(exp, attendance, achievements, position);
      } catch {
        conditionMet = false;
      }
...
const Dashboard = {
  render() {
    ...
    const learning = data.learningProgress?.[member.studentId] || {
      viewed: [],
      quizPassed: [],
    };
    const savedOpportunities =
      data.opportunities?.filter((item) =>
        item.savedBy?.includes(member.studentId)
      ) || [];
    const nearestOpportunity = [...savedOpportunities].sort(
      (a, b) => new Date(a.deadline) - new Date(b.deadline)
    )[0];
    const attendance =
      data.checkIns?.filter((item) => item.memberId === member.studentId)
        .length ||
    ...
    if (Auth.isOfficer()) {
      const pendingGrievance = data.grievances.filter(
        (g) => g.status === "submitted" || g.status === "in-progress"
      ).length;
      const pendingGradeConv = data.members.filter(
        (m) => m.gradeConvRequested
      ).length;
...
const DashboardV2 = {
  render() {
    const data = Storage.getAppData();
    const member = data.members.find(
      (m) => m.studentId === Auth.currentUser?.studentId
    );
    if (!member) return;
    const level = Gamification.getLevel(member.exp);
    const progress = Gamification.getProgress(member.exp);
    ...
    document.getElementById("content").innerHTML = `
      ...
      <section class="dashboard-section leaderboard-preview"><div class="section-title"><div><p class="eyebrow">COMMUNITY</p><h3>Leaderboard</h3></div><button class="text-action" onclick="window.__showLeaderboard()">See all</button></div>
        ${ranked
          ...
          .map(
            (m, index) =>
              `<div class="leader-row ${m.studentId === member.studentId ? "is-you" : ""}"><span class="leader-medal">${["🥇", "🥈", "🥉"][index]}</span><span class="leader-avatar">${sanitizeHTML(m.name.charAt(0))}</span><span><strong>${sanitizeHTML(m.name)}${m.studentId === member.studentId ? " <small>(You)</small>" : ""}</strong><small>Level ${Gamification.getLevel(m.exp).level}</small></span><b>${m.exp} EXP</b></div>`
          )
          .join("")}
      </section>
...
const EventsLegacy = {
  ...
  checkin(eventId) {
    ...
    const result = Gamification.addExp(
      user.studentId,
      event.exp,
    ...
    UI.toast({
      message: `✅ Checked in to ${event.title}! +${event.exp} EXP`,
      type: "success",
    });
    UI.confetti({ count: 40 });
...
const EventsLegacyRsvp = {
  render() {
    ...
    const scheduled = upcoming.filter(
      (event) => event.rsvps?.[userId] === "going"
    );
    document.getElementById("content").innerHTML = `
      <section class="my-schedule-card"><div><p class="eyebrow">MY SCHEDULE</p><h3>${scheduled.length ? `You’re going to ${scheduled.length} upcoming event${scheduled.length === 1 ? "" : "s"}.` : "Build your event schedule."}</h3><p>${
        scheduled.length
          ? scheduled
              .slice(0, 2)
              .map((event) => `${sanitizeHTML(event.title)} · ${event.date}`)
              .join("<br>")
          : "RSVP “Going” to see confirmed events here."
      }</p></div><span class="material-symbols-rounded">calendar_month</span></section>
    ...
    document
      ...
      .forEach((button) =>
        button.addEventListener("click", () =>
          this.rsvp(Number(button.dataset.eventId), button.dataset.rsvp)
        )
      );
    document
      .getElementById("event-search")
      ?.addEventListener("input", this.filterList);
    document
      .getElementById("event-filter")
      ?.addEventListener("change", this.filterList);
  },
  renderCard(event, userId) {
    const past = new Date(event.date) < new Date();
    const status = event.rsvps?.[userId] || "";
    const going = Object.values(event.rsvps || {}).filter(
      (value) => value === "going"
    ).length;
    const capacity = event.capacity || 50;
  ...
  rsvp(eventId, status) {
    ...
    if (
      status === "going" &&
      event.rsvps[userId] !== "going" &&
      going >= (event.capacity || 50)
    )
      return UI.toast({
        message: "This event is full. You can still choose Maybe.",
        type: "warning",
      });
  ...
  downloadCalendar(eventId) {
    const event = Storage.getAppData().events.find(
    ...
    UI.toast({ message: "Calendar file downloaded.", type: "success" });
  },
  checkin(eventId) {
    const user = Auth.currentUser;
    if (!user) return;
    ...
    if (!event || !member)
      return UI.toast({ message: "Event not found.", type: "error" });
    const today = new Date().toISOString().split("T")[0];
...
const Events = {
  get data() {
    return Storage.getAppData();
  },
  render() {
    const data = this.data,
      userId = Auth.currentUser?.studentId;
    const events = [...data.events].sort(
      (a, b) => new Date(a.date) - new Date(b.date)
    );
    const going = events.filter(
      (e) => new Date(e.date) >= new Date() && e.rsvps?.[userId] === "going"
    );
    document.getElementById("content").innerHTML =
      `<section class="my-schedule-card"><div><p class="eyebrow">MY SCHEDULE</p><h3>${going.length ? `You are going to ${going.length} upcoming event${going.length === 1 ? "" : "s"}.` : "Build your event schedule."}</h3><p>${
        going.length
          ? going
              .slice(0, 2)
              .map((e) => `${sanitizeHTML(e.title)} · ${e.date}`)
              .join("<br>")
          : "RSVP Going to place an event on your schedule."
    ...
    document
      ...
      .forEach((b) =>
        b.addEventListener("click", () =>
          this.rsvp(Number(b.dataset.eventId), b.dataset.rsvp)
        )
      );
    document
      .getElementById("event-search")
  ...
  card(event, userId) {
    const past = new Date(event.date) < new Date(),
      status = event.rsvps?.[userId] || "";
    const attendees = Object.entries(event.rsvps || {})
      .filter(([, value]) => value === "going")
      .map(([id]) => this.data.members.find((m) => m.studentId === id)?.name)
      .filter(Boolean);
    const capacity = event.capacity || 50,
      full = attendees.length >= capacity && status !== "going";
    const confirmed = this.data.checkIns?.some(
      (i) => i.memberId === userId && i.eventId === event.id
    );
  ...
  filter() {
    const q =
        document.getElementById("event-search")?.value.toLowerCase() || "",
      type = document.getElementById("event-filter")?.value || "all";
    document
      .querySelectorAll("#event-list .event-card")
      .forEach(
        (c) =>
          (c.style.display =
  ...
  rsvp(eventId, status) {
    ...
    if (!event || !userId) return;
    event.rsvps ||= {};
    const count = Object.values(event.rsvps).filter(
      (x) => x === "going"
    ).length;
    if (
      status === "going" &&
      event.rsvps[userId] !== "going" &&
      count >= (event.capacity || 50)
    )
      return UI.toast({
        message: "This event is full. You can still select Maybe.",
        type: "warning",
      });
    event.rsvps[userId] = status;
    Storage.saveAppData(data);
    UI.toast({
      message: status === "going" ? "Added to My Schedule." : "RSVP updated.",
      type: "success",
    });
    this.render();
  },
  ...
  checkin(eventId) {
    const user = Auth.currentUser,
      data = this.data,
      event = data.events.find((e) => e.id === eventId),
      member = data.members.find((m) => m.studentId === user?.studentId);
    if (!event || !member)
      return UI.toast({ message: "Event not found.", type: "error" });
    if (
      data.checkIns?.some(
        (i) => i.memberId === user.studentId && i.eventId === eventId
      )
    )
      return UI.toast({
        message: "Attendance is already confirmed for this event.",
        type: "warning",
      });
    const today = new Date().toISOString().split("T")[0];
    member.attendance ||= [];
    ...
    Storage.updateQuestProgress("checkin");
    UI.toast({
      message: `Attendance confirmed. +${event.exp} EXP`,
      type: "success",
    });
    this.render();
  },
  schedule() {
    const data = this.data,
      ...
      entries = data.events
        .filter(
          (e) =>
            ["going", "maybe"].includes(e.rsvps?.[userId]) &&
            new Date(e.date) >= new Date()
        )
        .sort((a, b) => new Date(a.date) - new Date(b.date));
    document.getElementById("content").innerHTML =
      `<section class="page-intro"><p class="eyebrow">MY SCHEDULE</p><h2>Your registered events</h2><p>Set a reminder and add any event to your phone calendar.</p></section>${entries.length ? entries.map((e) => `<article class="schedule-item"><div><strong>${sanitizeHTML(e.title)}</strong><p>📅 ${e.date} · ${sanitizeHTML(e.location || "TBA")}</p><span class="status">${e.rsvps[userId] === "going" ? "Going" : "Maybe"}</span></div><div><label class="reminder-label">Reminder <select data-reminder="${e.id}"><option value="none">None</option><option value="1 day">1 day before</option><option value="1 hour">1 hour before</option></select></label><button class="calendar-btn" onclick="window.__downloadEventCalendar(${e.id})">Add to calendar</button></div></article>`).join("") : UI.emptyState("Your schedule is empty. RSVP to an event to get started.", "event", "__nav('events')")}`;
    document.querySelectorAll("[data-reminder]").forEach((s) => {
      const e = data.events.find((x) => x.id === Number(s.dataset.reminder));
      s.value = e.reminders?.[userId] || "none";
      s.addEventListener("change", () => {
        e.reminders ||= {};
        e.reminders[userId] = s.value;
        Storage.saveAppData(data);
        UI.toast({
          message:
            s.value === "none"
              ? "Reminder removed."
              : `Reminder set for ${s.value}.`,
          type: "success",
        });
      });
    });
  ...
  feedback(eventId) {
    const event = this.data.events.find((e) => e.id === eventId);
    UI.modal(
      `<h3>Event feedback</h3><p>How was ${sanitizeHTML(event?.title || "the event")}?</p><form id="event-feedback-form"><label>Rating <select id="feedback-rating"><option value="5">5 — Excellent</option><option value="4">4 — Good</option><option value="3">3 — Okay</option><option value="2">2 — Needs work</option><option value="1">1 — Poor</option></select></label><label>Comments<textarea id="feedback-comment" placeholder="What should we keep or improve?"></textarea></label><button class="btn-primary" type="submit">Submit feedback</button></form>`
    );
    document
      .getElementById("event-feedback-form")
      ?.addEventListener("submit", (e) => {
        e.preventDefault();
        const data = this.data;
        data.eventFeedback.push({
          id: Date.now(),
          eventId,
        ...
        Storage.saveAppData(data);
        UI.closeModal();
        UI.toast({ message: "Thanks for your feedback!", type: "success" });
      });
  },
};
...
const Opportunities = {
  render() {
    const data = Storage.getAppData(),
      userId = Auth.currentUser?.studentId;
    const items = [...data.opportunities].sort(
      (a, b) => new Date(a.deadline) - new Date(b.deadline)
    );
    document.getElementById("content").innerHTML =
      `<section class="page-intro"><p class="eyebrow">OPPORTUNITIES HUB</p><h2>Open doors for your next chapter</h2><p>Scholarships, internships, debate invitations, volunteer work, and deadlines in one place.</p></section><div class="opportunity-filters">${["All", "Scholarship", "Internship", "Debate", "Volunteer"].map((type) => `<button data-opportunity-filter="${type}" class="${type === "All" ? "selected" : ""}">${type}</button>`).join("")}</div><div id="opportunity-list">${items.map((item) => this.card(item, userId)).join("")}</div>`;
      ...
      button.addEventListener("click", () => {
        ...
        document
          ...
          .forEach(
            (card) =>
              (card.style.display =
                button.dataset.opportunityFilter === "All" ||
                card.dataset.type === button.dataset.opportunityFilter
                  ? ""
                  : "none")
          );
      })
...
const Portfolio = {
  render() {
    ...
    if (!member) return;
    const checkIns =
        data.checkIns?.filter((x) => x.memberId === member.studentId) || [],
      badges = Gamification.getMemberBadges(member.studentId).filter(
        (x) => x.unlocked
      ),
      saved = data.opportunities.filter((x) =>
        x.savedBy?.includes(member.studentId)
      );
    const roles = member.position ? [member.position] : [],
      hours = checkIns.reduce(
        (total, checkIn) =>
          total +
          (data.events.find((e) => e.id === checkIn.eventId)?.type ===
          "volunteer"
            ? 4
            : 0),
        0
  ...
  certificate() {
    const data = Storage.getAppData(),
      ...
      count =
        data.checkIns?.filter((x) => x.memberId === member?.studentId).length ||
        0;
    const page = window.open("", "_blank");
    if (!page)
      return UI.toast({
        message: "Please allow pop-ups to download your certificate.",
        type: "warning",
      });
    page.document.write(
      `<title>Participation Certificate</title><style>body{font-family:Georgia;text-align:center;padding:80px;color:#29431d}h1{font-size:42px}main{border:8px solid #759954;padding:65px;max-width:700px;margin:auto}small{color:#555}</style><main><h1>Certificate of Participation</h1><p>This certifies that</p><h2>${sanitizeHTML(member.name)}</h2><p>has actively participated in SEPOLSCIS activities, with <strong>${count}</strong> confirmed event attendance record${count === 1 ? "" : "s"}.</p><p>Issued ${new Date().toLocaleDateString()}</p><br><small>SEPOLSCIS · Student Excellence in Political Science</small></main>`
    );
    page.document.close();
    page.focus();
...
const Grievance = {
  render() {
    ...
    if (!isOfficer) {
      grievances = grievances.filter(
        (g) => g.memberId === user.studentId || g.anonymous
      );
    }
  ...
  resolve(id) {
    ...
    document
      .getElementById("resolve-submit")
      ?.addEventListener("click", function () {
        const notes = document.getElementById("resolution-text").value.trim();
        const result = Storage.updateGrievance(id, {
          status: "resolved",
...
const Learning = {
  ...
  topics: [
    {
      ...
      icon: "gavel",
      takeaway:
        "The Constitution is the country’s highest law and protects fundamental rights.",
    },
    {
      id: "constitution1899",
      title: "1899 Malolos Constitution",
      category: "constitutions",
      icon: "history",
      takeaway:
        "It established the First Philippine Republic and placed sovereignty in the people.",
    },
    {
      id: "constitution1935",
      title: "1935 Commonwealth",
      category: "constitutions",
      icon: "history",
      takeaway:
        "It created the Commonwealth government that prepared the country for independence.",
    },
    {
      id: "constitution1943",
      title: "1943 Occupation",
      category: "constitutions",
      icon: "history",
      takeaway:
        "It was adopted during the Japanese occupation of the Philippines.",
    },
    {
      id: "constitution1973",
      title: "1973 Constitution",
      category: "constitutions",
      icon: "history",
      takeaway: "It changed the structure of government during the Marcos era.",
    },
    {
      id: "constitution1986",
      title: "1986 Freedom Constitution",
      category: "constitutions",
      icon: "history",
      takeaway:
        "It was a provisional constitution after the People Power Revolution.",
    },
    {
      id: "government",
      ...
      takeaway:
        "The legislative, executive, and judicial branches have distinct responsibilities.",
    },
    {
      id: "news",
      ...
      icon: "newspaper",
      takeaway:
        "Connect reliable reporting to political-science concepts and evidence.",
    },
    {
      id: "bylaws",
      title: "SEPOLSCIS By-Laws",
      category: "organization",
      icon: "groups",
      takeaway:
        "Know the organization’s rules, rights, responsibilities, and opportunities.",
    },
    {
      id: "sepolscisConstitution",
      title: "SEPOLSCIS Constitution",
      category: "organization",
      icon: "description",
      takeaway:
        "Read the complete constitution and bylaws, including membership, officers, elections, and finances.",
    },
    {
      id: "reviewer",
      title: "Exam Reviewer",
      ...
      icon: "school",
      takeaway:
        "Use the reviewer to refresh core political-science concepts before assessments.",
    },
  ],
  quizzes: {
    constitution: {
      question: "Where does sovereignty reside under the 1987 Constitution?",
      options: [
        "In Congress",
        "In the people",
        "In the President",
      ...
      answer: 1,
      explanation: "Article II states that sovereignty resides in the people.",
    },
    constitution1899: {
      question: "What did the Malolos Constitution establish?",
      options: [
    ...
    government: {
      ...
      options: ["Legislative", "Executive", "Judicial", "Local government"],
      answer: 2,
      explanation: "The judicial branch interprets laws and resolves cases.",
    },
    news: {
      question: "Which source is best for checking a public policy claim?",
      options: [
        "An anonymous post",
      ...
      answer: 1,
      explanation:
        "Start with primary and official sources, then compare credible reporting.",
    },
  },
  getProgress() {
    const data = Storage.getAppData();
  ...
  toggleSaved(id) {
    const progress = this.getProgress();
    progress.saved = progress.saved.includes(id)
      ? progress.saved.filter((topic) => topic !== id)
      : [...progress.saved, id];
    this.saveProgress(progress);
    this.render();
  },
  render() {
    ...
    const active =
      this.topics.find((topic) => topic.id === this.currentTopic) ||
      filtered[0] ||
      this.topics[0];
    document.getElementById("content").innerHTML = `
    ...
    document.querySelectorAll("[data-learning-category]").forEach((button) =>
      button.addEventListener("click", () => {
        this.currentCategory = button.dataset.learningCategory;
        this.render();
      })
    );
    document.querySelectorAll("[data-topic]").forEach((button) =>
      button.addEventListener("click", () => {
        this.currentTopic = button.dataset.topic;
        this.markViewed(this.currentTopic);
        this.render();
      })
    );
    document
      .getElementById("learning-search-input")
  ...
  renderQuiz(topic, quiz, progress) {
    if (progress.quizPassed.includes(topic.id))
      return '<div class="quiz-complete"><span class="material-symbols-rounded">workspace_premium</span><div><strong>Quiz complete</strong><p>Nice work—this topic is now marked complete.</p></div></div>';
  ...
  answerQuiz(topicId, answer) {
    ...
    if (answer !== quiz.answer)
      return UI.toast({
        message: "Not quite—try again and reread the key takeaway.",
        type: "warning",
      });
    const progress = this.getProgress();
    if (!progress.quizPassed.includes(topicId)) {
      ...
      Gamification.addExp(
        ...
        20,
        `Completed ${this.topics.find((topic) => topic.id === topicId)?.title} quiz`
      );
      UI.confetti({ count: 25 });
    }
    UI.toast({
      message: `Correct! ${quiz.explanation} +20 EXP`,
      type: "success",
    });
    this.render();
  },
...
const Officer = {
  renderDashboard() {
    ...
    const totalBadges = data.members.reduce(
      (sum, m) => sum + (m.badges?.length || 0),
      0
    );
  ...
  renderAnalytics() {
    ...
    const totalAttendances = data.members.reduce(
  ...
  showAwardBadge() {
    ...
    document
      .getElementById("award-badge-form")
      ?.addEventListener("submit", function (e) {
        e.preventDefault();
        ...
        if (!member)
          return UI.toast({ message: "Member not found.", type: "error" });
        ...
        UI.closeModal();
        UI.toast({
          message: `🏅 Badge awarded to ${member.name}!`,
          type: "success",
        });
        UI.confetti({ count: 50 });
        NotifCenter.add(
          "Badge Awarded",
          `${member.name} received a new badge!`,
          "success",
          "workspace_premium"
        );
  ...
  showAnnouncementForm() {
    ...
    document
      ...
      ?.addEventListener("submit", function (e) {
        ...
        if (!text)
          return UI.toast({
            message: "Please write an announcement.",
            type: "error",
          });
        ...
        const currentScreen =
          document.querySelector(".nav-item.active")?.dataset.screen || "home";
  ...
  createEvent() {
    ...
    document.getElementById("event-date").value = tomorrow
      .toISOString()
      .split("T")[0];
...
const App = {
  init() {
    ...
    this.setupPWA();
    document
      .getElementById("officer-menu-btn")
      ?.classList.toggle("hidden", !Auth.isOfficer());
    ...
    try {
      ...
      State.subscribe("appData", (data) => {
        NotifCenter.updateBadge();
      });
    } catch (e) {
      // State may not be available in some contexts
      console.warn("State subscriptions failed", e);
    }
  ...
  setupListeners() {
    ...
    window.__viewGrievances = () => {
      this.navigate("events");
      setTimeout(() => Grievance.render(), 100);
    };
    window.__viewGradeRequests = () => {
      this.navigate("events");
    ...
    window.__requestGrade = () => {
      ...
      Storage.setCurrentUser(Auth.currentUser);
      UI.toast({
        message: "Grade conversion request submitted!",
        type: "success",
      });
    ...
    window.__updateGrievance = (id, status) =>
      Grievance.updateStatus(id, status);
    ...
    window.__downloadReviewer = () => {
      ...
      const data = Storage.getAppData();
      const content =
        data.learningResources?.reviewer || "Reviewer content not available.";
    ...
    window.__answerLearningQuiz = (id, answer) =>
      Learning.answerQuiz(id, answer);
  ...
  handleQRCheckin() {
    ...
    if (upcoming.length === 0) {
      return UI.toast({
        message: "No upcoming events to check in to.",
        type: "warning",
      });
    }
