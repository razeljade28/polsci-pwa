...
export const Portfolio: React.FC<PortfolioProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Premium Banner */}
      <section className="bg-gradient-to-br from-zinc-950 via-blue-950 to-indigo-950 rounded-3xl p-6 text-white shadow-lg border border-zinc-800 relative overflow-hidden">
        <div className="absolute right-0 top-0 w-36 h-36 bg-gradient-to-bl from-blue-500/10 to-transparent rounded-full filter blur-xl" />
        <div className="absolute -left-12 -bottom-12 w-28 h-28 border-4 border-white/5 rounded-full" />

        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            <span className="text-[10px] text-blue-300 font-extrabold uppercase tracking-widest block">SMC PolSci Verified Record</span>
          ...
          <button
            ...
            className="w-full md:w-auto px-5 py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black shadow-md flex items-center justify-center gap-2 cursor-pointer transition-transform hover:-translate-y-0.5"
          >
      ...
      <section className="grid grid-cols-3 gap-3">
        {[
          { icon: Calendar, value: attendanceRecords.length, label: "Attended Events", color: "text-blue-600 bg-blue-50 dark:bg-blue-950/20" },
          { icon: Award, value: unlockedBadges.length, label: "Earned Badges", color: "text-amber-600 bg-amber-50 dark:bg-amber-950/20" },
          { icon: Clock, value: `${volunteerHours}h`, label: "Volunteer Hours", color: "text-purple-600 bg-purple-50 dark:bg-purple-950/20" }
        ].map((stat, i) => (
      ...
      <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm space-y-4">
        <div>
          <span className="text-[10px] text-blue-600 dark:text-blue-400 font-extrabold uppercase tracking-wider block">Honor Society</span>
            ...
            return (
              ...
              >
                ...
                <span className={`text-[9px] font-black uppercase mt-3 tracking-wider block ${
                  unlocked ? "text-blue-600 dark:text-blue-400" : "text-zinc-400"
                }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm space-y-4">
          <div>
            <span className="text-[10px] text-blue-600 dark:text-blue-400 font-extrabold uppercase tracking-wider block">Chronicle</span>
            <h3 className="text-sm font-extrabold text-zinc-900 dark:text-zinc-100 mt-0.5 font-display">
              📅 Attendance Logs
            </h3>
          </div>
              ...
              {attendanceRecords.map((record) => {
                ...
                return (
                  <div key={record.eventId} className="flex justify-between items-center p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800">
                    ...
                    <span className="text-[11px] font-extrabold text-blue-600 dark:text-blue-400">+{record.exp} EXP</span>
                  </div>
                );
              })}
            </div>
        ...
        <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-sm space-y-4">
          <div>
            <span className="text-[10px] text-blue-600 dark:text-blue-400 font-extrabold uppercase tracking-wider block">Bookmarks</span>
            <h3 className="text-sm font-extrabold text-zinc-900 dark:text-zinc-100 mt-0.5 font-display">
              📌 Saved Opportunities
            </h3>
          </div>
              ...
              {savedOpportunities.map((opp) => (
                <div key={opp.id} className="p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 space-y-1">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="text-[10px] font-extrabold uppercase text-amber-800 bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded border border-amber-200/40">
                      {opp.type}
                    </span>
                  </div>
